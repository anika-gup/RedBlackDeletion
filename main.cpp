...
bool casethree(node* &root, node* current, node* parent, node* sibling, node* closenephew, node* distantnephew);
  ...
  if (current->getValue()==number) {
	...
	while (current!=root && current->getColor()>0) {
	  ...
	  if (sibling!=NULL && sibling->getColor()==0) {
	    ...
	    //case3
	    if (casethree(root, current, parent, sibling, closenephew, distantnephew)==true) {
	      del(toDelete);
	      return;
	    }
	    else{
	  ...
	  if (parent!=NULL && parent->getColor()==0) {
	    // go to case D4
	    //goto Case_D4;
	    cout << "PRINTING BEFORE CASE FOUR" << endl;
	    PRINT(root, 0);
	    casefour(root, current, parent, sibling, closenephew, distantnephew);

	    del(toDelete);
	    cout << "PRINTING AFTER CASE FOUR" << endl;
	    PRINT(root, 0);
	    return;
	  }
	...
	sp->setRight(s->getRight());
      ...
      */
      ...
      cout << "swapped vales," << endl;
      //current->setValue(value);
      PRINT(root, 0);
      REMOVE(root, s, s->getValue());
      current->setValue(value);
      if (current==root) {
	root->setValue(value);
      }
...
void casefive(node* &root, node* current, node* parent, node* sibling, node* closenephew, node* distantnephew) {
  ...
  PRINT(root, 0);
  cout << "finished case 5 print" << endl;
  casesix(root, current, parent, sibling, closenephew, distantnephew);
}
...
void casesix(node* &root, node* current, node* parent, node* sibling, node* closenephew, node* distantnephew) {
  ...
  PRINT(root, 0);
  cout << "finished case 6 print" << endl;
  return;
}
...
void del(node* toDelete) {
  cout << "Deleting: " << toDelete->getValue() << endl;
  int c = toDelete->getColor();
