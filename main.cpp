...
void REMOVE(node* &root, node* current, int number) {
  ...
  if (current->getValue()==number) {
	...
	/*
	while(db->getColor()==2) {
	  //while it's double black
	  if (db==root) {
	    db->setColor(1);
	    break;
	  }
	  // trying to find db sibling.
	  if (db->getValue() > db->getParent()->getValue()) {
	    // sibling is on the left.
	    node* sibling = db->getParent()->getLeft();
	  }
	  else {
	    //sibling is on the right.
	    node* sibling = db->getParent()->getRight();
	  }
	  if (sibling->getColor()==0) {
	    // case 4, sibling is red.
	    // code this in !!-------------------
	  }
	  // don't run it without finishing this otherwise infinite loop --------------------
	}
